...
namespace sudsol
{
    ...
    public:
        Board();
        ~Board();
        bool GetHorizontalLine(uint16_t x, std::vector<std::shared_ptr<Cell>>& cell_ptrs, std::string& err_message) const;
        bool GetVerticalLine(uint16_t y, std::vector<std::shared_ptr<Cell>>& cell_ptrs, std::string& err_message) const;
